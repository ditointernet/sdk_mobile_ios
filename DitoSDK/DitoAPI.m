...
+(void)configure:(NSString *)apiKey secret:(NSString *)secret {
    kApiKey = apiKey;
    kSecret = secret;
    kSignature = [self encriptCredentials];
    internetDown = NO;
...
+(void)request:(NSString *)module path:(NSString *)path params:(NSMutableDictionary *)params requestType:(HttpTypes)requestType completion:(void (^)(id, NSError *))block {
    if ([[self trimmingString:module] isEqualToString:@""]) {
        if (block) {
    ...
    if (![[self trimmingString:path] isEqualToString:@""]) {
    ...
    if (![method isEqualToString:@"GET"]) {
        ...
        if (![[self trimmingString:json] isEqualToString:@""]) {
...
+(void)notificationRead:(DitoCredentials *)credentials message:(NSString *)message completion:(void (^)(id, NSError *))block {
    if (![self validateData:block]) {
        return;
    } else if (!message || [[self trimmingString:message] isEqualToString:@""]) {
        if (block) {
...
+(NSString *)getNetwork:(DitoCredentials *)credentials {
    ...
    if ([self trimmingString:credentials._id] && ![[self trimmingString:credentials._id] isEqualToString:@""]) {
        return @"portal";
    } else if ([self trimmingString:credentials.facebookID] && ![[self trimmingString:credentials.facebookID] isEqualToString:@""]) {
        return @"facebook";
    } else if ([self trimmingString:credentials.twitterID] && ![[self trimmingString:credentials.twitterID] isEqualToString:@""]) {
        return @"twitter";
    } else if ([self trimmingString:credentials.googlePlusID] && ![[self trimmingString:credentials.googlePlusID] isEqualToString:@""]) {
        return @"plus";
    }
...
+(NSString *)getIdentifyKey:(DitoCredentials *)credentials {
    ...
    } else if ([self trimmingString:credentials.reference] && ![[self trimmingString:credentials.reference] isEqualToString:@""]) {
        return @"reference";
    }
...
+(BOOL)validateUseReference:(void (^)(id, NSError *))block method:(NSString *)methodName credentials:(DitoCredentials *)credentials {
    if (([methodName isEqualToString:@"identify"] || [methodName isEqualToString:@"alias"] || [methodName isEqualToString:@"unalias"]) && ([self trimmingString:credentials.reference] && ![[self trimmingString:credentials.reference] isEqualToString:@""])) {
        if (block) {
...
+(void)peformRequest:(HttpTypes)requestType url:(NSString *)url params:(NSMutableDictionary *)params completion:(void (^)(id, NSError *))block {
    ...
    __block NSData *responseData;// = [NSURLConnection sendSynchronousRequest:request returningResponse:&urlResponse error:&error];
